...
const bulletRef = {
  ...
  bulletObjArr: bulletObjArr,
  enemyObjArr: enemy_obj_arr,
...
function bullet() {
  if (bulletObjArr.length <= 0) {
    spacePressed = false;
    return;
  }
  bulletObjArr.map(bullet => {
    ctx.beginPath();
    ...
    if (bullet.y > -1) {
      ...
      bulletObjArr = bulletObjArr.filter(item => item !== bullet);
    }
...
function shootBullet() {
  Bullet(bulletRef).shoot();

  console.log(spacePressed);
  // const bullet_inst = Bullet(player.x, player.y, player.y);
  // bulletObjArr.push(bullet_inst);
}
